...
class Database:
    ...
    async def set_dump_chat(self, id, chat_id):
        value = None if chat_id is None else int(chat_id)
        await self.col.update_one({'id': int(id)}, {'$set': {'dump_chat': value}})
    ...
    async def remove_replace_words(self, id, words):
        ...
        await self.col.update_one({'id': int(id)}, {'$set': {'replace_words': current_repl}})
    # Filter Mode Support
    async def set_filter_mode(self, id, mode):
        await self.col.update_one({'id': int(id)}, {'$set': {'filter_mode': mode}})
    async def get_filter_mode(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('filter_mode', 'all') if user else 'all'
    # Daily Limits
