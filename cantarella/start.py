...
def apply_text_rules(text, delete_words=None, replace_words=None):
    """Apply the user's replace rules first, then remove delete words."""
    if not text:
        return text or ""

    result = text
    for old, new in sorted((replace_words or {}).items(), key=lambda item: len(item[0]), reverse=True):
        if old:
            result = re.sub(re.escape(old), str(new), result, flags=re.IGNORECASE)

    for word in sorted(delete_words or [], key=len, reverse=True):
        if word:
...
def apply_filename_rules(file_path, delete_words, replace_words):
    """Rename the downloaded file so replacements also affect Telegram filenames."""
    if not file_path:
        return file_path

    directory, filename = os.path.split(file_path)
    new_filename = apply_text_rules(filename, delete_words, replace_words).strip()
    if not new_filename:
        new_filename = "download"
    ...
    try:
        ...
        logger.error(f"Failed to rename downloaded file {file_path}: {exc}")
        return file_path


def render_caption(template, filename, size):
    try:
        return template.format(filename=filename, size=size)
    ...
    for msgid in range(fromID, toID + 1):
        ...
        if is_public_link:
            ...
            try:
                ...
                # Copy is fast and preserves albums/entities, but it cannot apply
                # captions, filenames, delete words, or replacement words.
                if not custom_caption and not delete_words and not replace_words:
                    ...
                    # Use the normal processing pipeline whenever the user has
                    # enabled any content transformation.
                    await handle_restricted_content(
                        client, client, message, username, msgid
                    )
            except Exception as e:
                logger.error(f"Error processing public message {username}/{msgid}: {e}")
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid):
    ...
    if msg_type == "Text":
        try:
            text = apply_text_rules(msg.text, delete_words, replace_words)
            await client.send_message(
                target_chat,
                text,
                parse_mode=enums.ParseMode.HTML
            ...
            logger.error(f"Failed to send text message to {target_chat}: {e}")
            return
    ...
    try:
        ...
        file = apply_filename_rules(file, delete_words, replace_words)
    ...
    try:
        ...
        if custom_caption:
            ...
            if msg.caption:
                final_caption += f"\n\n{msg.caption}"
        final_caption = apply_text_rules(
            final_caption,
            delete_words,
            replace_words
        )
